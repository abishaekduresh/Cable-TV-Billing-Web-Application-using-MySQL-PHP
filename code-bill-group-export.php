...
if (isset($_POST['submit'])) {
    ...
    $sheet->setCellValue('J1', 'Rs');
    $sheet->setCellValue('K1', 'Status');
    // $sheet->setCellValue('L1', 'Bill id');

    $row = 2;
    while ($row_data = $result->fetch_assoc()) {
        $sheet->setCellValue('A' . $row, $row_data['billNo']);
        $sheet->setCellValue('B' . $row, $row_data['date']);
        $sheet->setCellValue('C' . $row, "'" . $row_data['billBy']);
        ...
        $sheet->setCellValue('H' . $row, $row_data['paid_amount']);
        $sheet->setCellValue('I' . $row, $row_data['discount']);
        $sheet->setCellValue('J' . $row, $row_data['Rs']);
        $sheet->setCellValue('K' . $row, $row_data['status']);
        // $sheet->setCellValue('L' . $row, $row_data['bill_id']);
        $row++;
    }
    ...
    $filename = 'Group_Bill_Data_' . $session_username . '_' . $currentDate . '_' . $currentTime . '.xls';
