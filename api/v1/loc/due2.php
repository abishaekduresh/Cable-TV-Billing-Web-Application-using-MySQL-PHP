...
header('Access-Control-Allow-Origin: https://pdpgroups.in'); // Replace with your specific domain for production
header('Access-Control-Allow-Methods: POST,OPTIONS'); // Allow specific methods
header('Access-Control-Allow-Headers: Content-Type'); // Allow specific headers

// Handle preflight requests
if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    exit; // exit if the request is a preflight request
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($missing_fields)) {
		...
		// $update_loc_prop_login_limit = update_loc_prop_login_limit($token);
		$update_loc_prop_login_limit = "true";
		
		if($update_loc_prop_login_limit === "true"){
			...
			$get_due_month_year_by_gen_bill_id_array = get_due_month_year_by_gen_bill_id($loc_gen_bill_id,$channel_uid);
			$due_month = $get_due_month_year_by_gen_bill_id_array['due_month'];
			$due_year = $get_due_month_year_by_gen_bill_id_array['due_year'];
			$active_status = 1;
...
*/			$zero_status = 0;
			$stmt = $con->prepare("
				...
				WHERE 
					loc_gen_bills_log.status = ?
					AND loc_gen_bills.status = ?
					AND loc_gen_bills.due_status = ?
					AND loc_bills.status = ?
					AND loc_gen_bills.channel_uid = ?
					AND loc_bills.channel_uid = ? ORDER BY loc_gen_bills_log.due_year
			");

			// Bind parameters (two channel_uid used twice, so binding twice)
			$stmt->bind_param("iiiiss", $active_status, $active_status, $zero_status, $active_status, $channel_uid, $channel_uid);
			// Execute the query
			$stmt->execute();
			$result = $stmt->get_result();
