...
$stbno = validateInput($data['stbno']);
$phone = validateInput($data['phone']);

$requiredFields = ['stbno', 'phone'];
...
$stmt = $con->prepare("SELECT name, phone, rc_dc FROM customer WHERE stbno = ? AND phone = ?");
if (!$stmt) {
    $response = array(
        "status" => "failed",
...
// Bind parameters
if (!$stmt->bind_param("si", $stbno, $phone)) {
    $response = array(
        "status" => "failed",
...
$stmt->bind_result($cus_name, $phone, $rc_dc);
...
if($rc_dc === 0){
    ...
    $response = array(
        "status" => "success",
        "message" => "Invalid STB Number or Phone Number",
        "code" => "201"
    );
    echo json_encode($response);
    exit;
