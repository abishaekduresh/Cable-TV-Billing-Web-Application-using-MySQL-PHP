...
function get_loc_channel_by_uid($channel_uid) {
    ...
    // // Prepare the SQL statement based on whether $status and/or $channel_uid are provided
    // if ($status !== null && $channel_uid !== null) {
    //     // Both $status and $channel_uid are provided
    //     $stmt = $con->prepare("SELECT * FROM loc_channels WHERE channel_uid = ? AND status = ?");
    //     $stmt->bind_param("si", $channel_uid, $status);
    // } elseif ($status !== null) {
    //     // Only $status is provided
    //     $stmt = $con->prepare("SELECT * FROM loc_channels WHERE status = ?");
    //     $stmt->bind_param("i", $status);
    // } elseif ($channel_uid !== null) {
    //     // Only $channel_uid is provided
    //     $stmt = $con->prepare("SELECT * FROM loc_channels WHERE channel_uid = ?");
    //     $stmt->bind_param("s", $channel_uid);
    // } else {
    //     // Neither $status nor $channel_uid is provided
...
// function get_loc_gen_bills_log($loc_gen_bill_log_id = null) {
//     global $con;
//     $status = 1;

//     if($loc_gen_bill_log_id = null){
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills_log WHERE loc_gen_bill_log_id = ? AND status = ? LIMIT 1");
//         $stmt->bind_param("ii", $loc_gen_bill_log_id, $status);
//     }else{
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills_log WHERE status = ?");
//         $stmt->bind_param("i", $status);
//     }
...
//         return array('error' => 'No record found');
//     }
...
// // function get_loc_gen_bills($channel_uid, $loc_gen_bill_log_id, $status) {
// function get_loc_gen_bills($channel_uid = null, $loc_gen_bill_log_id = null, $status = null) {
//     global $con;
//     global $currentMonth, $currentYear;

//     // Prepare the SQL statement based on whether $status and/or $channel_uid are provided
//     if ($status !== null && $channel_uid !== null && $loc_gen_bill_log_id !== null) {
//         // All parameters are provided
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills WHERE loc_gen_bill_log_id = ? AND channel_uid = ? AND status = ?");
//         $stmt->bind_param("isi", $loc_gen_bill_log_id, $channel_uid, $status);
//     // } elseif ($status !== null && $channel_uid !== null) {
//     //     // Both $status and $channel_uid are provided
//     //     $stmt = $con->prepare("SELECT * FROM loc_gen_bills WHERE channel_uid = ? AND status = ?");
//     //     $stmt->bind_param("si", $channel_uid, $status);
//     } elseif ($loc_gen_bill_log_id !== null) {
//         // Only $status is provided
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills WHERE loc_gen_bill_log_id =? AND status = ?");
//         $stmt->bind_param("i", $loc_gen_bill_log_id);
//     } elseif ($channel_uid !== null) {
//         // Only $channel_uid is provided
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills WHERE channel_uid = ?, status = ?");
//         $stmt->bind_param("si", $channel_uid, $status);
//     }else{
//         $stmt = $con->prepare("SELECT * FROM loc_gen_bills WHERE MONTH(created_at) == ? 
//                                     AND YEAR(created_at) == $currentYear AND status = ?");
//         $stmt->bind_param("iii", $currentMonth, $currentYear, $status);
//     }
...
// function get_loc_bills($channel_uid = null, $loc_gen_bill_id = null, $status = null) {
//     global $con;

//     // Prepare the SQL statement based on whether $status and/or $channel_uid are provided
//     if ($status !== null && $channel_uid !== null && $loc_gen_bill_id !== null) {
//         // All parameters are provided
//         $stmt = $con->prepare("SELECT *, SUM(paid_amount) AS paid_amount FROM loc_bills WHERE loc_gen_bill_id = ? AND channel_uid = ? AND status = ?");
//         $stmt->bind_param("isi", $loc_gen_bill_id, $channel_uid, $status);
//     } elseif ($status !== null && $channel_uid !== null) {
//         // Both $status and $channel_uid are provided
//         $stmt = $con->prepare("SELECT * FROM loc_bills WHERE channel_uid = ? AND status = ?");
//         $stmt->bind_param("si", $channel_uid, $status);
//     } elseif ($status !== null) {
//         // Only $status is provided
//         $stmt = $con->prepare("SELECT * FROM loc_bills WHERE status = ?");
//         $stmt->bind_param("i", $status);
//     } elseif ($channel_uid !== null) {
//         // Only $channel_uid is provided
//         $stmt = $con->prepare("SELECT * FROM loc_bills WHERE channel_uid = ?");
//         $stmt->bind_param("s", $channel_uid);
//     }
...
function loc_sms_api($phone, $token = null) {

    // if($message_code === "1"){
        $message = rawurlencode('Dear Customer, To check your Cable TV LOC payment due details, visit: pdpgroups.in/due.php/?t='.$token.' For queries, contact help@dureshtech.com. Software by DURESH TECH.');
    // }else{
    //     $message = 'Null';
    // }
    ...
    $apiKey = urlencode('CpyZ6bykjbpXgAhnqSP');
    $sender_id = urlencode('DURTEH');
    $template_id = urlencode('1707172710513175735');
